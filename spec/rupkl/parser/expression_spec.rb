...
RSpec.describe RuPkl::Parser do
  ...
  describe 'method call' do
    it 'should be parsed by expression parser' do
      ...
      expect(parser)
        .to parse('foo?.bar()')
        .as(method_call(member_ref(:foo), :bar, nullable: true))
      expect(parser)
        .to parse('foo.bar(1)')
      ...
      expect(parser)
        .to parse('foo?.bar?.baz()')
        .as(method_call(member_ref(member_ref(:foo), :bar, nullable: true), :baz, nullable: true))
      expect(parser)
        .to parse('foo?.bar.baz()')
        .as(method_call(member_ref(member_ref(:foo), :bar, nullable: true), :baz))
      expect(parser)
        .to parse('foo.bar?.baz()')
        .as(method_call(member_ref(member_ref(:foo), :bar), :baz, nullable: true))
      expect(parser)
        .to parse('foo.bar.baz(1)')
      ...
      expect(parser)
        .to parse('foo().bar().baz()')
        .as(method_call(method_call(method_call(:foo), :bar), :baz))
      expect(parser)
        .to parse('foo.bar().baz.qux()')
        .as(method_call(member_ref(method_call(member_ref(:foo), :bar), :baz), :qux))
      expect(parser)
        .to parse('"foo".bar()')
      ...
      expect(parser)
        .to parse('null?.bar()')
        .as(method_call(null_expression, :bar, nullable: true))
    end
  ...
  describe 'member reference' do
    it 'should be parsed by expression parser' do
      ...
      expect(parser)
        .to parse('foo?.bar')
        .as(member_ref(member_ref(:foo), :bar, nullable: true))
      expect(parser)
        .to parse('foo.bar.baz')
        .as(member_ref(member_ref(member_ref(:foo), :bar), :baz))
      expect(parser)
        .to parse('foo?.bar?.baz')
        .as(member_ref(member_ref(member_ref(:foo), :bar, nullable: true), :baz, nullable: true))
      expect(parser)
        .to parse('foo?.bar.baz')
        .as(member_ref(member_ref(member_ref(:foo), :bar, nullable: true), :baz))
      expect(parser)
        .to parse('foo.bar?.baz')
        .as(member_ref(member_ref(member_ref(:foo), :bar), :baz, nullable: true))
      expect(parser)
